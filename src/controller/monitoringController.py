...
def finalize_monitoring(db: Session, monitoring_id: int, monitoring: MonitoringUpdate):
    # Verificar si el monitoreo existe
    db_monitoring = db.query(Monitoring).filter(Monitoring.id == monitoring_id).first()
    if db_monitoring is None:
        raise HTTPException(status_code=404, detail="Monitoreo no encontrado")
    ...
    if not monitoring.fecha_finalizacion:
        raise HTTPException(status_code=422, detail="El campo 'fecha_finalizacion' es obligatorio para finalizar el monitoreo.")

    # Actualizar los campos específicos para finalizar
    db_monitoring.estado = 2  # Cambiar estado a "Finalizado"
    db_monitoring.fecha_finalizacion = monitoring.fecha_finalizacion
